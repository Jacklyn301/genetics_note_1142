...
$$\frac{\text{表現出性狀的人數}}{\text{攜帶致病基因的人數}}\times 100\%$$
...
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/chimerism_and_mosacism_0602.webp)
...
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/liability_distribution_0325.png)

- 平均值M=0，在親代群體中，我們將平均的liability值設定為0作為基準點
- 其中，M* 代表已經患病的親代平均liabiity值
- M' 代表觀察子代之後，那些患病的親代所生之子代的平均liability
- 通常來說，子代的M'會比親代的M=0來的高，但通常有兩種情況:
  - 1. M'非常接近M* ，代表該性狀遺傳力很高。
  - 2. M'比較接近M=0附近，該性狀更多受環境影響，而不是基因
 
#### threshold出現差異
 - 按理來說，門檻值應該在各個族群都是一樣的，但是當族群拆成男女來看的時候，兩個性別的liability分佈即使相同，疾病發病率依然可能有差異
 - 因此，我們在這時會讓兩方的threshold有差異，例如，如果男性那方的門檻值較低，代表疾病發生率通常也更高

#### 雙生子研究
- 同卵雙胞胎 (monozygote, MZ) 在基因上是一樣的，因此如果他門之間有表徵上的差異，只能歸咎於環境的影響。例如研究養育於不同家庭的雙胞胎，有助於區分環境跟遺傳的影響
> [!Note]
> dizygote, DZ，異卵雙胞胎，只共享了約50%的基因，類似於兄弟姊妹 🐱

- 透過同時研究DZ跟MZ雙胞胎，看看他們對某些疾病的發生頻率
- 兩個雙胞胎同時表現某一特徵或疾病的程度，被稱為一致率 (concordance)，對於主要由基因決定的東西，MZ的一致性往往高於DZ，通常理論上來說:
  - 如果某種特徵或是疾病**完全由基因決定**，那在一致率上，**MZ就是100%，DZ就是50%**
  - 相反，如果**沒有遺傳差異**，那麼MZ跟DZ的一致率應該會**一樣**
 
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/concordance_chart_0325.png)

- 基因型跟環境變異產生的是綜合效應。假如說某個疾病的基因型分為 AA、Aa、aa，而你能算出兩組數據:
  - 一個是僅受到基因型影響，環境不變的族群分佈，其變異數為 $\sigma^2_g$
  - 一個是僅受到環境變異影響，基因型不變的族群分佈，其變異數為 $\sigma^2_e$
  - 那麼在環境跟基因共同的影響下，其變異數其實就是兩者相加，又被稱為總表型變異，以 $\sigma^2_p$ 表示，公式如下:

$$\sigma^2_p=\sigma^2_g + \sigma^2_e$$

> [!Caution]
> 這有一個前提，就是我們假設基因是基因，環境是環境，他們沒有交互作用 (但通常這不一定，所以往往還要加上一個交互作用項) 🤔

![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/phenotypic_variance_0325.png)

#### heritability
- 遺傳度 ( $H^2$ )，估計一個群體中，某一性狀的表型變異裡面，多少是因為遺傳因素決定的，是一個針對 "族群" 的統計量
- 公式通常為:

$$H^2=\frac{V_G}{V_P}$$

- 其中， $V_G$ 代表基因造成的變異， $V_P$ 代表總表型變異 (也就是包含基因 + 環境)，其值介於0到1之間
- 遺傳的形式是不會變的 (例如你是多基因遺傳還是體染色體基因遺傳啥的)，但是遺傳度會變，而且往往是因為環境變化而變
  - 例如，在越平等的社會裡面，IQ的遺傳度就會越高 (就代表環境影響智力的程度因為平等社會而變低)
  - 一些身體特徵的遺傳度通常都不低，例如內翻足 (0.8)、身高 (0.8) 等等

#### coefficient of correlation
- 遺傳學中，相關係數常用於測量親屬之間，某一性狀的相似程度，或兩個具有親屬關係的個體共享基因的比例，又稱為親緣係數 (coefficient of relatedness)，通常用 $r$ 表示

|關係|關係度|相關係數|
|---|---|---|
|兄弟姊妹之間|$1^\circ$|50% (1/2)|
|父母和孩子之間|$1^\circ$|50% (1/2)|
|叔叔阿姨對姪子姪女|$2^\circ$|25% (1/4)|
|祖父母跟孫子孫女|$2^\circ$|25% (1/4)|
|表兄弟姊妹之間|$3^\circ$|12.5% (1/8)|

- 在家族研究中，用來估計疾病的遺傳風險。親緣係數越高，若疾病有遺傳基礎，則共病的機率也越高
- 此數據也可以用於親代投資理論，也就是Hamilton’s rule:

$$rB>C$$

- 如果利他行為對親屬的好處乘上親緣係數大於成本，這種行為就可能被自然選擇保留下來。

#### empiric risk
- "經驗風險" 代表根據族群中觀察到的疾病發生率，估算某個疾病再發(recurrence) 的機率，可以用於流行病學的研究
- 經驗風險通常來自於觀察資料，而不是利用理論推算
> [!Tip]
> 所以它是一個看當下事實會變來變去的值，單純猜測你下一胎一樣中鏢的機率而已 ! 👀

- 經驗風險受到各種因子影響。

##### 受影響的家庭成員數
- 該家族**患病人數**越多，整個家族的liability curve就會更右偏，經驗風險越高，接下來出生的sibiling就更可能發病
- 假如說有個夫妻生下一個有神經管缺陷的小孩，再發風險大概是2~4%。如果是他們有兩個神經管缺陷的孩子，再發風險就變成10%

> [!Note]
> 這感覺有點像是: 你中了第二次，那我懷疑第三次會中的機率也會一起上升。 🙂
> 並非生下第二個孩子讓你增加了風險，而是 "兩個孩子有神經管缺陷" 的事實揭露了這對夫婦在liability分佈中原本就處於高風險的位置

##### 病情的嚴重程度
- 如果該家族**患病的人病情嚴重**，其下一胎再發的風險就會比較高
- 假如說一對夫妻生下的小孩有單邊兔唇，那麼下一胎的再發風險是2%。但如果它是雙邊兔唇，下一胎的再發風險大約是6%
> [!Note]
> 因為如果該疾病屬多基因遺傳，妳的孩子病情嚴重，相當於我們猜測，基因池的風險因子頻率更多
> 也就是，你的孩子病情嚴重，相當於他中了比較多標，我們就懷疑家族裡面本來就容易中標! 🤔

##### 血緣關係
- 如果被影響的人其血緣關係跟病人越近，再發風險就越高。近親風險遠高於遠親，因為近親能分享更多的liability

#### 臨床疾病復發風險分析
- 我們可以透過將患病的盛行率和理論曲線比對，判定疾病身可能的遺傳性值
> 咱們來解析以下圖片看看...

![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/recurrence_risks_of_common_medical_conditions_0325.png)

- 橫軸為該疾病在一般群體的盛行率，縱軸代表患者的一級親屬 (例如他的孩子) 的患病風險
- 頂端兩條水平線是**單基因遺傳疾病**的理論風險，這些疾病風險跟遺傳的盛行率無關，僅由遺傳定律決定
   - 如果你是體染色體顯性，那就是50%，如果是體染色體隱性，那就是25%
- 其餘的斜線代表符合liability threshold model的多因素遺傳疾病
   - 這些疾病復發風險會隨著群體盛行率升高而升高 (代表整體的分佈往右邊移動)
   - 斜線的y截距越大，該疾病受遺傳因素控制的程度越高 (也就是 $h^2$ 越高啦)
- 如果某疾病落於水平線上，那我們只需鎖定單一基因，即可篩檢
- 如果是落在斜線上，如內翻足 (clubfoot)、epilipsy、diabetes等，需要考慮更多微效基因和環境因子的累積效應
- 通常如果某個疾病在一般人群中越罕見，其相對風險倍數會越高
- 如果想考量家族史對於疾病的影響權重有多少，可以看看該疾病落於哪條 $h^2$ 線附近 

---

## chapter 18
### introduction of population genetics
- 族群遺傳學，是用來研究在演化的影響下，等位基因的分佈跟改變，看看在世代交替的影響之下，維持或是改變基因型頻率的因素
- 不同的alleles在不同族群間有比例 (也就是所謂的 "頻率" ) 上的差異

### Hardy–Weinberg equilibrium
- 哈溫平衡是一個 "假設alleles的比例在每一次世代交替下，都會差不多" 的理想世界 (柏拉圖會按讚 👍)。有好幾個前提:
   - 族群近乎無限大
   - 族群內隨機交配，沒有偏好
   - 等位基因頻率在雌雄個體間相同
   - 沒有突變發生
...
#### 咱們來看一下數學模型 👀
- 假如說有兩個alleles: $A$ 跟 $a$，並且頻率如下: 

$$[A]=p,[a]=q\rightarrow p+q=1$$

- 然後我們展開二項式，預測三種基因型的頻率: $AA$ 、 $Aa$ 、 $aa$ 

$$
\begin{align}
& [AA]=p^2,\ [Aa]=2pq,\ [aa]=q^2\\
\rightarrow\quad & \boxed{p^2+2pq+q^2=1}
\end{align}
$$

- 在哈溫平衡條件下，alleles跟genotype的頻率會保持穩定
- 演化就是基因型頻率偏離哈溫平衡的現象，又稱為microevolution (微演化)

#### chi-square test也可以檢測
> 假如說一族群的genotype: $MM=1787,\ MN=3037,\ NN=1305$ ，有沒有脫離平衡? 🤔

- 假說設立:

$$
\begin{align}
& H_0\rightarrow\text{該族群沒有偏離哈溫平衡}\\
& H_a\rightarrow\text{該族群偏離哈溫平衡}
\end{align}
$$

- 我們先來看看總數跟期望值 ( $E$ ) 應該怎麼樣:

$$
\begin{align}
& 1787+3037+1305=6129\\
& p_{M}=\frac{(1787+\frac{3037}{2})}{6129}=0.539\\
& q=1-0.539=0.461\\
& p^2=0.539^2=0.2905\rightarrow 0.2905\times 6129=1781\\
& 2pq=2\times 0.539\times 0.461=0.4970\rightarrow 0.4970\times 6129=3046\\
& q^2=0.461^2=0.2125\rightarrow 0.2125\times 6129=1299\\
\end{align}
$$
...
$$\chi^2=\frac{(1787-1781)^2}{1781}+\frac{(3037-3046)^2}{3046}+\frac{(1305-1299)^2}{1299}=0.0489$$

- 查表 ( $df=1$ ) 發現該統計對應p值為0.9，接受虛無假說 $H_0$ ，**沒有偏離哈溫平衡**

#### 基因型頻率分佈
- 假如說我們思考在p和q變動時不同基因型的頻率，我們會發現:
  - 異型合子頻率的峰值出現在p=q=0.5時 (此時族群中有一半是異型合子)
  - 等位基因頻率介於1/3和2/3之間時，異型合子是群體中最常見的基因型
  - 當一個基因變得罕見時，其carrier往往多於患者，異型合子的數量遠超過隱性同型合子，當q的頻率下降時， $q^2$ 曲線下降速度遠超過 $2pq$ 的曲線
  - 這能說明為什麼有害的基因難以透過自然選擇 "完全剔除"，因為這些基因會藏在健康的carriers身上

![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/allele_frequency_0326.png) 

### mating patterns
- 通常配對模式分為隨機分配跟非隨機分配

#### random mating
- 個體跟特定基因型配對的機率=該基因型在族群中的頻率
- 無選擇性的配對，純粹靠運氣

#### non-random mating
- 在人類族群中，配對往往具有選擇性，這被稱為assortative mating

##### positive assortative mating
- 選擇跟自己的表徵或是遺傳背景相近的配偶
- cosanguinity (也就是近親結婚) 就是經典例子之一

##### negative assortiative mating
- 選擇跟自己的表徵不一樣的配偶

#### cosanguinity 的遺傳效應
- 近親結婚本身不會改變等位基因的頻率，也就是說， $p$ 跟 $q$ 本身不變，但是基因型頻率會變化: 也就是 $aa$ 或是 $AA$ 的頻率增加， $Aa$ 的頻率下降
- 這會導致純合子比例上升，而雜合子比例下降。這種現象稱為 inbreeding depression (近交衰退)。

##### 隱性有害基因的顯現
- 在一般族群中，許多有害突變以隱性形式存在，不會表現出來
- 近親交配提高了隱性基因成為純合子的機率，使隱藏的有害隱性表型顯現，導致疾病或適應力下降，從而接受自然選擇的篩選

> [!Note]
> 具體來說，近親繁殖會導致 $AA$ 和 $aa$ 的基因型頻率幾乎跟p跟q一樣
- 這也能說明為何某些遺傳病在近親交配族群中更常見。
- 圖中藍色曲腺 (非親屬配對) 可以發現，符合哈溫平衡預期，隱性同型合子的頻率隨著 $q^2$ 下降而減少
- 而紅色曲腺 (近親交配)，隱性同型合子的頻率明顯高於非親屬配對
- 當 $q=0.01$ 時，藍色曲腺幾乎貼近為0，但是紅色仍有顯著高度，這代表對於罕見疾病來說，近親交配的風險可能是普通人的數十倍

![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/cousin_mating_0331.png)

- 除了動物之外，植物也會受到其影響，隨著近交係數增加，玉米的產量也會顯著降低

### 遺傳漂變 grnetic drift
- 遺傳漂變指的是基因頻率因為隨機事件而改變，而不是因為自然選擇
- 在有限族群中，等位基因的傳遞受到 "隨機性" 影響。
- 某些等位基因可能因為偶然因素 (例如某些個體很衰沒留下後代) 而消失或固定。這種隨機波動就是**遺傳漂變**

#### 特性
- 遺傳漂變的強度跟族群的規模成反比: 族群個體數量越少，遺傳漂變的效應就越強 

> [!Note]
> 想像一下:
> - 在大族群中，每一代的基因傳遞就像擲硬幣。如果擲 1000 次，正反面比例會很接近 1:1 
> - 但如果只擲 4 次 (代表小族群)，很可能出現 4 次都是正面 (代表某等位基因固定或消失)，這就是抽樣導致的漂變
#### 產生的影響
- 遺傳漂變可能導致某些allele完全消失，進而降低族群的遺傳多樣性
- 遺傳漂變也是導致子群之間基因頻率產生差異的主要原因。因為小族群的基因頻率波動大、速度快，所以在短時間內就可能出現顯著差異。
- 這些差異累積下來，就造成族群間的**遺傳分化**

#### 大族群跟小族群差別

- 遺傳漂變是隨機的，不同小族群各自受到隨機抽樣影響，基因頻率偏移方向不一樣。因此，每個族群的基因組成逐漸不同 → 族群間分化
- 但是如果把所有族群加總起來看，整體的基因頻率平均值仍然維持在原本的水平。因為漂變只是隨機波動，沒有方向性，不會讓整個大族群的平均基因頻率 "系統性偏移"

> [!Important]
> 漂變只是增加了族群間的差異，但不會改變整體平均值。 🐱

![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/genetic_drift_0331.png)

#### bottleneck effect
- 指突然發生某些災難 (例如氣候變化、過度捕獵)，導致物種群劇烈減少50%以上或是數量級減少的事件
- 災難後的倖存著只能攜帶親代基因池中的一小部分變異
- 這會加劇遺傳漂變、可能導致族群滅絕，或是即使族群恢復，其遺傳多樣性也非常有限

![image alt](https://vectormine.b-cdn.net/wp-content/uploads/Bottleneck_Effect_outline_diagram_2-576x432.jpg)

#### founder effect
- 由少數個體 (通常帶有特定突變) 重新在一個地區建立新的群體，由於跟外界隔絕，個體彼此間的基因差異性較小
- 常發生在地理隔絕的小島，或是較為封閉的新開闢村落等等

![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/founder_effect_0331.png)

#### founder mutation
- 一種會導致疾病的，位於染色體上的基因，往往是來自於一個 **"共同染色體祖先"**
- 通常該致病基因在族群規模較小的時候引入，而且由於族群的封閉或是隔離，導致其出現的頻率變高
...
### natural selection
- 被偏好的allele頻率增加，不被偏好的allele頻率減少
- 具有有利變異的後帶可以在生存競爭中存活，透過各代累積，使後代跟親代有所不同

#### 顯性跟隱性基因
- 顯性基因通常在選擇壓力下，其頻率會迅速改變
- 隱性基因在選擇壓力下，其影響比較緩慢
...
flowchart TB
subgraph 自然選擇的四大條件
A[繁殖能力<br>任何生物單位具有複製自身的能力]
B[子代數量多<br>子代的數目要夠多 ]
C[存活差異<br>子代的存活決定於某些特徵]
D[可遺傳性<br>這些特徵具有遺傳傳遞的機制]
end
...
|type|stabilizing selection|directional selection|disrupive selection|
|---|---|---|---|
|概念|環境有利於表現型性狀分布線的平均值附近，淘汰兩側較極端的個體|選擇對一側的極端個體有利，使族群平均值往該側移動|選擇對兩側極端的個體有利，不利於中間個體|
|例如|人類嬰兒的出生體重過輕跟過重都會導致死亡，因此多數嬰兒體重落於平均值|大部分人工選擇會做的手段|可能導致族群分化，或是形成新物種|

![image alt](https://image1.slideserve.com/3326546/mechanisms-of-natural-selection-l.jpg)

#### mutation
- 所有遺傳變異的根本來源，但是由於突變率很低，短時間內改變基因頻率的力量相對較弱
- 多數突變對生物體有害，會在自然選擇中被剔除
- 突變的原因可能是因為化學物質 (被稱為mutagen)，或著是輻射
...
> 當**新突變產生的速度 = 天擇剔除的速度**，這被稱為 **"突變與選擇平衡"** 🐱
